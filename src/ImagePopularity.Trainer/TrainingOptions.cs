...
internal sealed class TrainingOptions
{
    ...
    public required string OutputModelPath { get; init; }

    public bool OutputModelPathAutoGenerated { get; init; }
    ...
    public static TrainingOptions Parse(string[] args)
    {
        ...
        var outputModelPath = map.TryGetValue("output-model", out var outputModel)
            ? outputModel
            : BuildDefaultOutputModelPath(trainImageSize, enableAugmentation, epochs, batchSize, seed);
        ...
        var options = new TrainingOptions
        {
            ...
            OutputModelPathAutoGenerated = !map.ContainsKey("output-model"),
...
Notes:
  ...
  If --output-model is omitted, the trainer auto-generates a short file name
  containing train sample count, image size, augmentation flag, epochs,
...
""";
    ...
    private static string BuildDefaultOutputModelPath(
        int trainImageSize,
        ...
        int seed)
    {
        ...
        var fileName =
            $"{trainImageSize}_{augmentationTag}_e{epochs}b{batchSize}s{seed}.pt";
    ...
    {
        if (!OutputModelPathAutoGenerated)
        {
            return OutputModelPath;
        }

        var directory = Path.GetDirectoryName(OutputModelPath);
        var extension = Path.GetExtension(OutputModelPath);
        var augmentationTag = EnableAugmentation ? "a1" : "a0";
        var baseName = $"{trainSampleCount}_{TrainImageSize}_{augmentationTag}_e{Epochs}b{BatchSize}s{Seed}";
        var timestamp = completedAtLocal.ToString("MMddHHmm", CultureInfo.InvariantCulture);
